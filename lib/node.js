...
{
	var ethUrl = 'http://' + RPC_HOST + ':' + (process.env.RPC_PORT || '8545');
			...
			peers: function (callback)
			{
				...
				request(netInfoUrl, function response(error, response, body) {
					if (!error && response.statusCode == 200) {
						...
						callback(null, info.result[1].peers.length);
					} else {
						callback(error, null);
					}
				});
...
Node.prototype.getPending = function()
{
	...
	if (this._web3)
	{
		...
		request(unconfirmedUrl, function response(err, response, body) {
			if (err || response.statusCode != 200) {
				console.error('xx>', 'getPending error: ', err);
				return false;
			}
			var info = JSON.parse(body);
			var pending = info.result[1].n_txs;
